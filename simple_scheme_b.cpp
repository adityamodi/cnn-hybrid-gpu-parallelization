...
int main(int argc, char** argv)
{
    ...
    {   
        ...
        checkCudaErrors(cudaMalloc(&d_gconv1,     sizeof(float) * NUM_WORKERS_DATA_PARALLELISM * conv1.pconv.size()));
        checkCudaErrors(cudaMalloc(&d_gconv1bias, sizeof(float) * NUM_WORKERS_DATA_PARALLELISM * conv1.pbias.size()));
        checkCudaErrors(cudaMalloc(&d_gconv2,     sizeof(float) * NUM_WORKERS_DATA_PARALLELISM * conv2.pconv.size()));
        checkCudaErrors(cudaMalloc(&d_gconv2bias, sizeof(float) * NUM_WORKERS_DATA_PARALLELISM * conv2.pbias.size()));
        ...
        for(int iter = 0; iter < ITERATIONS; ++iter)
        {
            ...
            // TODO: Use wait to receive the go message from the previous worker for sequential
            ...
            // TODO: Use individual waits to send message to other if not the first processor
    ...
    if(proc_id >= NUM_WORKERS_DATA_PARALLELISM && proc_id < num_processors)
    {
        ...
        FullyConnectedLayer fc2(FC1_OUTPUT_DIM, my_class_size);
        ...
        checkCudaErrors(cudaMalloc(&d_dfc2smax, sizeof(float) * fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM * fc2.outputs));
        checkCudaErrors(cudaMalloc(&d_dlossdata,sizeof(float) * fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM * fc2.outputs));
        ...
        float *d_pool2_fc_all, *d_pool2_fc, *d_labels_fc_all, *d_labels_fc;
        // float *d_pool2_all, *d_pool2_fraction_1, *d_pool2_fraction_2;
        checkCudaErrors(cudaMalloc(&d_pool2_fc_all,   sizeof(float) * fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM * CONV2_FILTERS * CONV2_OUTPUT_DIM * CONV2_OUTPUT_DIM));
        ...
        checkCudaErrors(cudaMalloc(&d_labels_fc_all,   sizeof(float) * fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM));
        ...
        float *d_dfc2_all;
        checkCudaErrors(cudaMalloc(&d_dfc2_all, sizeof(float) * fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM * NUM_WORKERS_DATA_PARALLELISM * fc2.inputs));
        ...
        for (int iter = 0; iter < ITERATIONS; ++iter){
            ...
            for(int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++ ){
                ...
                MPI_Irecv(d_labels_fc + i * fc_context.m_batchsize, MPI_FLOAT, i, FORPROP_LABELS_TAG, MPI_COMM_WORLD, &req_labels[i]);
            }
            ...
            count_req = 0;
            for( int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++){
                len = fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM * (FC1_OUTPUT_DIM / NUM_WORKERS_DATA_PARALLELISM);
            ...
            //float *labels, float *fc1relu, float *fc2, float *fc2smax, float *dloss_data,
            // float *pfc2, float *pfc2bias, float *gfc2, float *gfc2bias,
            // float *dfc2, float *onevec
            fc_context.BackpropagationLogisticLayer((int)my_class, d_labels_fc, d_fc2, d_fc2smax, d_dlossdata, d_pfc2, d_pfc2bias,
            ...
            for(int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++)
            {
                len = fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM * fc2.inputs;
                if(proc_id == i + NUM_WORKERS_DATA_PARALLELISM) continue;
            ...
            fc_context.BackpropagationFullyConnectedLayer(d_pool2_fc, d_fc1, d_fc1relu, d_dfc1, d_dfc1relu, d_dfc2,
                                                          d_pfc1, f_pfc1bias, d_gfc1, d_gfc1bias, d_onevec);
            ...
            //  float *gfc1, float *gfc1bias,
            //  float *gfc2, float *gfc2bias
            fc_context.UpdateWeights(STEP_SIZE, d_pfc1, d_pfc1bias, d_pfc2, d_pfc2bias, d_gfc1, d_gfc1bias, d_gfc2, d_gfc2bias);
