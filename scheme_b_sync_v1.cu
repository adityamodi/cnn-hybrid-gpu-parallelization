...
#define NUM_WORKERS_DATA_PARALLELISM 4
...
__global__ void PutValToVector(double val, float *vec, int idx)
{
    vec[idx] = (float)val;
}
...
__global__ void copyData(const float *data1, float *data2, int len){
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void ComputeLogisticActivations(float *fc2, int num_output_units, int batch_size, float *result)
{
    ...
    // result[idx] = 1.0 / (1.0 + exp(-1.0 * fc2[idx]));
      result[idx] = 1.0 / (1.0 + exp(-1.0 * mySharedVariable[threadIdx.x]) + 0.0000001);
}
...
__global__ void ComputeLoss(const float *labels, const float *predictions, float *loss, int start_label, int my_class_size, int batch_size)
{
    ...
    if(labels[img_idx] == start_label + idx % my_class_size)
        loss[img_idx] = -log(predictions[img_idx + idx % my_class_size] + 0.0000001);
}
...
__global__ void AccumulateLoss(float *d_loss_all, float *d_loss_accumulated, int num_workers)
{
    for(int i = 1; i < num_workers; i++)
    {
        d_loss_all[0] += d_loss_all[i];
    }
    d_loss_accumulated[0] += d_loss_all[0];
}
...
struct TrainingContextFullyConnectedLayer
{
    ...
    TrainingContextFullyConnectedLayer(int gpuid, int batch_size, FullyConnectedLayer& fc1, FullyConnectedLayer& fc2) : ref_fc1(fc1), ref_fc2(fc2), m_gpuid(gpuid), m_batchsize(batch_size)
    {
        ...
        checkCUDNN(cudnnSetActivationDescriptor(fc1Activation, CUDNN_ACTIVATION_CLIPPED_RELU,
                                                CUDNN_NOT_PROPAGATE_NAN, 10.0));
    }
...
int main(int argc, char** argv)
{
    ...
    {   
        ...
        int num_features_per_image, num_samp;;
        std::string fname = "mnist_training_images_" + std::to_string(proc_id) + ".csv";
        std::ifstream in_file (fname);
        if(in_file.is_open())
        {
            in_file >> num_samp >> num_features_per_image;
        }
        train_size = num_samp;
        len = num_features_per_image;

        // std::vector<unsigned int> train_images(train_size * img_width * img_height * img_channels);
        // std::vector<unsigned int> train_labels(train_size);
        std::vector<unsigned int> train_images;
        std::vector<unsigned int> train_labels;
        ...
        if(in_file.is_open())
        {
            // in_file >> train_size >> num_features_per_image;
        ...
        std::chrono::time_point<std::chrono::high_resolution_clock> s_conv, e_conv, s_p2fc, e_p2fc, s_cgsync, e_cgsync;
        std::chrono::duration<double> iter_elap, p2fc_elap, cgsync_elap;
        double iter_time, p2fc_time, cgsync_time;

        float *d_iter_time_vector, *d_p2fc_time_vector,*d_cgsync_time_vector;
        checkCudaErrors(cudaMalloc(&d_iter_time_vector, sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL)));
        checkCudaErrors(cudaMalloc(&d_p2fc_time_vector, sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL)));
        checkCudaErrors(cudaMalloc(&d_cgsync_time_vector, sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL)));

        float *h_iter_time_vector = (float *)malloc((int)(ITERATIONS / TEST_INTERVAL) * sizeof(float));
        float *h_p2fc_time_vector = (float *)malloc((int)(ITERATIONS / TEST_INTERVAL) * sizeof(float));
        float *h_cgsync_time_vector = (float *)malloc((int)(ITERATIONS / TEST_INTERVAL) * sizeof(float));
        ...
        for(int iter = 0; iter < ITERATIONS; ++iter)
        {
            ...
            s_conv = std::chrono::high_resolution_clock::now();
            ...
            // printf("proc_id: %d  forward prop", proc_id);
            ...
            // printf("proc_id: %d  forward prop completed", proc_id);
            ...
            int go_msg;
            if(proc_id != 0) MPI_Recv(&go_msg, 1, MPI_INT, proc_id - 1, GO_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

            s_p2fc = std::chrono::high_resolution_clock::now();
            ...
            if( proc_id != NUM_WORKERS_DATA_PARALLELISM - 1){
                int stat = 1;
                MPI_Send(&stat, 1, MPI_INT, proc_id + 1, GO_TAG, MPI_COMM_WORLD);
            }
            MPI_Waitall(NUM_WORKERS_DATA_PARALLELISM, req, MPI_STATUSES_IGNORE);
            MPI_Waitall(NUM_WORKERS_DATA_PARALLELISM, req_labels, MPI_STATUSES_IGNORE);

            e_p2fc = std::chrono::high_resolution_clock::now();
            ...
            s_cgsync = std::chrono::high_resolution_clock::now();
            ...
            e_cgsync = std::chrono::high_resolution_clock::now();
            ...
            e_conv = std::chrono::high_resolution_clock::now();
            iter_elap = e_conv - s_conv;
            p2fc_elap = e_p2fc - s_p2fc;
            cgsync_elap = e_cgsync - s_cgsync;
            iter_time = iter_elap.count();
            p2fc_time = p2fc_elap.count();
            cgsync_time = cgsync_elap.count();

            PutValToVector<<<1, 1>>>(iter_time, d_iter_time_vector, (int)(iter + 1) / TEST_INTERVAL);
            PutValToVector<<<1, 1>>>(p2fc_time, d_p2fc_time_vector, (int)(iter + 1) / TEST_INTERVAL);
            PutValToVector<<<1, 1>>>(cgsync_time, d_cgsync_time_vector, (int)(iter + 1) / TEST_INTERVAL);
        }
        checkCudaErrors(cudaMemcpy(h_iter_time_vector, &d_iter_time_vector[0], sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL), cudaMemcpyDeviceToHost));
        std::ofstream myfile ("iter_time_proc_id_" + std::to_string(proc_id) + ".txt");
        if (myfile.is_open())
        {
            for(int idx = 0; idx < (int)(ITERATIONS / TEST_INTERVAL); idx++)
            {
                myfile << h_iter_time_vector[idx] << "\n";
            }
            myfile.close();
        }
        checkCudaErrors(cudaMemcpy(h_p2fc_time_vector, &d_p2fc_time_vector[0], sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL), cudaMemcpyDeviceToHost));
        std::ofstream myfile1 ("p2fc_proc_id_" + std::to_string(proc_id) + ".txt");
        if (myfile1.is_open())
        {
            for(int idx = 0; idx < (int)(ITERATIONS / TEST_INTERVAL); idx++)
            {
                myfile1 << h_p2fc_time_vector[idx] << "\n";
            }
            myfile1.close();
        }
        checkCudaErrors(cudaMemcpy(h_cgsync_time_vector, &d_cgsync_time_vector[0], sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL), cudaMemcpyDeviceToHost));
        std::ofstream myfile2 ("cgsync_proc_id_" + std::to_string(proc_id) + ".txt");
        if (myfile2.is_open())
        {
            for(int i = 0; i < (int)(ITERATIONS / TEST_INTERVAL); i++)
            {
                myfile2 << h_cgsync_time_vector[i] << "\n";
            }
            myfile2.close();
        }
    }
    if(proc_id >= NUM_WORKERS_DATA_PARALLELISM && proc_id < num_processors)
    {
        ...
        // printf("proc_id: %d\n", proc_id);
        checkCudaErrors(cudaSetDevice(gpu_rank));
        float my_class_size, my_class;

        std::chrono::time_point<std::chrono::high_resolution_clock> s_fc, e_fc, s_fpass, e_fpass, s_bp, e_bp, s_fc1, e_fc1, s_dfc1, e_dfc1, s_pool, e_pool;
        std::chrono::duration<double> fc_elap, fpass_elap, bp_elap, fc1_elap, dfc1_elap, pool_elap;
        double fc_time = 0, fpass_time = 0, bp_time = 0, fc1_time = 0, dfc1_time = 0, pool_time = 0;

        float *d_fc_time_vector, *d_fpass_time_vector, *d_bp_time_vector, *d_fc1_time_vector, *d_dfc1time_vector, *d_pool_time;
        checkCudaErrors(cudaMalloc(&d_fc_time_vector, sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL)));
        checkCudaErrors(cudaMalloc(&d_fpass_time_vector, sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL)));
        checkCudaErrors(cudaMalloc(&d_bp_time_vector, sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL)));
        checkCudaErrors(cudaMalloc(&d_fc1_time_vector, sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL)));
        checkCudaErrors(cudaMalloc(&d_dfc1time_vector, sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL)));
        checkCudaErrors(cudaMalloc(&d_pool_time, sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL)));

        float *h_fc_time_vector = (float *)malloc((int)(ITERATIONS / TEST_INTERVAL) * sizeof(float));
        float *h_fpass_time_vector = (float *)malloc((int)(ITERATIONS / TEST_INTERVAL) * sizeof(float));
        float *h_bp_time_vector = (float *)malloc((int)(ITERATIONS / TEST_INTERVAL) * sizeof(float));
        float *h_fc1_time_vector = (float *)malloc((int)(ITERATIONS / TEST_INTERVAL) * sizeof(float));
        float *h_dfc1time_vector = (float *)malloc((int)(ITERATIONS / TEST_INTERVAL) * sizeof(float));
        float *h_pool_time = (float *)malloc((int)(ITERATIONS / TEST_INTERVAL) * sizeof(float));
        ...
        FullyConnectedLayer fc1(CONV2_OUTPUT_DIM * CONV2_OUTPUT_DIM * CONV2_FILTERS, FC1_OUTPUT_DIM/NUM_WORKERS_DATA_PARALLELISM);
        ...
        TrainingContextFullyConnectedLayer fc_context(gpu_rank, BATCH_SIZE, fc1, fc2);
        ...
        checkCudaErrors(cudaMalloc(&f_class, sizeof(float)));
        checkCudaErrors(cudaMemcpyAsync(f_class, &my_class, sizeof(float), cudaMemcpyHostToDevice));
        ...
        float *d_gfc1_all, *d_gfc1bias_all, *d_gfc2_all, *d_gfc2bias_all;
        checkCudaErrors(cudaMalloc(&d_gfc1_all,       sizeof(float) * fc1.pneurons.size() * NUM_WORKERS_DATA_PARALLELISM));
        checkCudaErrors(cudaMalloc(&d_gfc1bias_all,   sizeof(float) * fc1.pbias.size() * NUM_WORKERS_DATA_PARALLELISM));
        checkCudaErrors(cudaMalloc(&d_gfc2_all,       sizeof(float) * fc2.pneurons.size() * NUM_WORKERS_DATA_PARALLELISM));
        checkCudaErrors(cudaMalloc(&d_gfc2bias_all,   sizeof(float) * fc2.pbias.size() * NUM_WORKERS_DATA_PARALLELISM));
        ...
        checkCudaErrors(cudaMalloc(&d_pool_all,sizeof(float) * fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM * CONV2_FILTERS * CONV2_OUTPUT_DIM * CONV2_OUTPUT_DIM)); // Changed
        ...
        checkCudaErrors(cudaMalloc(&d_labels_all,  sizeof(float) * fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM));

        float *d_fc1_all;
        checkCudaErrors(cudaMalloc(&d_fc1_all, sizeof(float) * fc_context.m_batchsize * FC1_OUTPUT_DIM));

        float *d_dfc2_all;
        checkCudaErrors(cudaMalloc(&d_dfc2_all, sizeof(float) * fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM * fc2.inputs));

        float *d_loss, *d_loss_all, *d_loss_sum, *d_loss_local_sum, *d_loss_accumulated;
        ...
        checkCudaErrors(cudaMalloc(&d_loss_accumulated, sizeof(float)));
        checkCudaErrors(cudaMalloc(&d_loss_all, sizeof(float) * NUM_WORKERS_DATA_PARALLELISM));
        checkCudaErrors(cudaMalloc(&d_loss_sum, sizeof(float) * (ITERATIONS / TEST_INTERVAL)));
        float *h_loss_sum = (float *)malloc((int)(ITERATIONS / TEST_INTERVAL) * sizeof(float));
        ...
        // checkCudaErrors(cudaMalloc(&d_pool2_fc_test,   sizeof(float) * (TEST_SET_SIZE / NUM_WORKERS_DATA_PARALLELISM) * CONV2_FILTERS * CONV2_OUTPUT_DIM * CONV2_OUTPUT_DIM));
        // checkCudaErrors(cudaMalloc(&d_labels_fc_test,   sizeof(float) * (TEST_SET_SIZE / NUM_WORKERS_DATA_PARALLELISM)));
        // float *d_fc1_all_test;
        // checkCudaErrors(cudaMalloc(&d_fc1_all_test, sizeof(float) * (TEST_SET_SIZE / NUM_WORKERS_DATA_PARALLELISM) * FC1_OUTPUT_DIM));
        // float *d_onevec_test;
        ...
        for (int iter = 0; iter < ITERATIONS; ++iter){
            ...
            //         MPI_Irecv(d_pool2_fc_test + i * len, len, MPI_FLOAT, i, FORPROP_TAG, MPI_COMM_WORLD, &req_conv_activations[i]);
            //         MPI_Irecv(d_labels_fc_test + i * TEST_SET_SIZE, TEST_SET_SIZE, MPI_FLOAT, i, FORPROP_LABELS_TAG, MPI_COMM_WORLD, &req_labels[i]);
            //     }
            ...
            //         len = TEST_SET_SIZE * NUM_WORKERS_DATA_PARALLELISM * fc1.outputs;
            ...
            //         if(proc_id == NUM_WORKERS_DATA_PARALLELISM)
            //         {
            //             ComputeZeroOneLoss<<<1, 1>>>(d_predictions_all, d_labels_fc_test + i, d_accuracy, NUM_WORKERS_DATA_PARALLELISM);
            //         }
            ...
            s_fc = std::chrono::high_resolution_clock::now();
            ...
            for(int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++ ){
                ...
                // MPI_Recv(d_pool2_fc + i * len, len, MPI_FLOAT, i, FORPROP_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                // MPI_Recv(d_labels_fc + i * fc_context.m_batchsize, MPI_FLOAT, i, FORPROP_LABELS_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Irecv(d_pool_all + i * len, len, MPI_FLOAT, i, FORPROP_TAG, MPI_COMM_WORLD, &req_conv_activations[i]);
                MPI_Irecv(d_labels_all + i * fc_context.m_batchsize, fc_context.m_batchsize, MPI_FLOAT, i, FORPROP_LABELS_TAG, MPI_COMM_WORLD, &req_labels[i]);
            }
            // fc_context.ForwardPropagationFullyConnectedLayer(d_pool2_all, d_fc1, d_fc1relu, d_pfc1, d_pfc1bias, d_onevec);
            ...
            SetScalarVal<<<1, 1>>>(d_loss_accumulated, 0.0);

            for(int j = 0; j < NUM_WORKERS_DATA_PARALLELISM; j++){
                s_pool = std::chrono::high_resolution_clock::now();
                MPI_Wait(&req_conv_activations[j], MPI_STATUS_IGNORE);
                MPI_Wait(&req_labels[j], MPI_STATUS_IGNORE);
                e_pool = std::chrono::high_resolution_clock::now();

                s_fpass = std::chrono::high_resolution_clock::now();

                len = fc_context.m_batchsize * CONV2_FILTERS * CONV2_OUTPUT_DIM * CONV2_OUTPUT_DIM;
                copyData<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_pool_all + j * len, d_pool2_fc, len);
                copyData<<<RoundUp(fc_context.m_batchsize, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_labels_all + j * fc_context.m_batchsize, d_labels_fc, fc_context.m_batchsize);
                fc_context.ForwardPropagationFullyConnectedLayer(d_pool2_fc, d_fc1, d_fc1relu, d_pfc1, d_pfc1bias, d_onevec);

                s_fc1 = std::chrono::high_resolution_clock::now();
                ...
                for( int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++){
                    ...
                    if(proc_id == i + NUM_WORKERS_DATA_PARALLELISM) continue;
                    MPI_Isend(d_fc1relu, len, MPI_FLOAT, i + NUM_WORKERS_DATA_PARALLELISM, FORPROP_TAG, MPI_COMM_WORLD, &req[count_req]);
                    count_req++;
                }
                MPI_Request req_recv[NUM_WORKERS_DATA_PARALLELISM - 1];
                count_req = 0;
                for( int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++){
                    len  = fc_context.m_batchsize * fc1.outputs;
                    if( proc_id == i + NUM_WORKERS_DATA_PARALLELISM ) copyData<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_fc1relu, d_fc1_all + i * len, len); // call copy kernel
                    // MPI_Recv(d_fc1_all + i * len, len, MPI_FLOAT, i + NUM_WORKERS_DATA_PARALLELISM, FORPROP_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    else
                    {
                        MPI_Irecv(d_fc1_all + i * len, len, MPI_FLOAT, i + NUM_WORKERS_DATA_PARALLELISM, FORPROP_TAG, MPI_COMM_WORLD, &req_recv[count_req]);
                        count_req++;
                    }
                }
                MPI_Waitall(NUM_WORKERS_DATA_PARALLELISM - 1, req, MPI_STATUSES_IGNORE);
                ...
                e_fc1 = std::chrono::high_resolution_clock::now();
                ...
                // ReduceLoss<<<1, 1>>>(d_loss, d_loss_local_sum, fc_context.m_batchsize);

                e_fpass = std::chrono::high_resolution_clock::now();
                s_bp = std::chrono::high_resolution_clock::now();
                ...
                s_dfc1 = std::chrono::high_resolution_clock::now();
                ...
                for(int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++)
                {
                    ...
                    // if(i != NUM_WORKERS_DATA_PARALLELISM - 1) len_d = fc_context.m_batchsize * fc2.inputs;
                    // else len_d = fc_context.m_batchsize * end_class_size;
                    if(proc_id == i + NUM_WORKERS_DATA_PARALLELISM) continue;
                ...
                for(int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++){
                    ...
                    // else len_d = fc_context.m_batchsize * end_class_size;
                    if(proc_id == i + NUM_WORKERS_DATA_PARALLELISM){
                        copyData<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_dfc2, d_dfc2_all + (i * len), len);
                    }
                    else
                    {
                       MPI_Irecv(d_dfc2_all + (i * len), len, MPI_FLOAT, i + NUM_WORKERS_DATA_PARALLELISM, BACKPROP_FC_TAG, MPI_COMM_WORLD, &fc1_recv[count_req]);
                       count_req++;
                    }
                }
                ...
                len = fc_context.m_batchsize * fc2.inputs;
                ReduceGradients<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_dfc2_all, d_dfc2, len, NUM_WORKERS_DATA_PARALLELISM);

                len = fc_context.m_batchsize * fc1.outputs;
                copyData<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_dfc2 + (proc_id - NUM_WORKERS_DATA_PARALLELISM) * len, d_dfc2_t, len);

                e_dfc1 = std::chrono::high_resolution_clock::now();

                // float *pool2, float *fc1, float *fc1relu, float *dfc1, float*dfc1relu, float *dfc2,
                // float *pfc1, float *pfc1bias, float *gfc1, float *gfc1bias, float *onevec
                // printf("fc proc_id: %d ====> 2nd backprop started\n", proc_id);
                MPI_Request req1;
                if(j != 0) MPI_Wait(&req1, MPI_STATUS_IGNORE);
                fc_context.BackpropagationFullyConnectedLayer(d_pool2_fc, d_fc1, d_fc1relu, d_dfc1, d_dfc1relu, d_dfc2,
                                                              d_pfc1, d_pfc1bias, d_gfc1, d_gfc1bias, d_onevec);
                // printf("fc proc_id: %d ====> 2nd backprop done\n", proc_id);

                e_bp = std::chrono::high_resolution_clock::now();

                
                // MPI_Status stat1[NUM_WORKERS_DATA_PARALLELISM];
                // for(int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++){
                len = fc_context.m_batchsize * fc1.inputs;
                ...
                MPI_Isend(d_dfc1, len, MPI_FLOAT, j, BACKPROP_TAG, MPI_COMM_WORLD, &req1);
                    // printf("fc proc_id: %d ====> IRecv started to address: %d\n", proc_id, i);
                // }
                len = fc1.pneurons.size();
                copyData<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_gfc1, d_gfc1_all + j*len, len);
                len = fc1.pbias.size();
                copyData<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_gfc1bias, d_gfc1bias_all + j*len, len);
                len = fc2.pneurons.size();
                copyData<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_gfc2, d_gfc2_all + j*len, len);
                len = fc2.pbias.size();
                copyData<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_gfc2bias, d_gfc2bias_all + j*len, len);
                            // float step_size,
                //  float *pfc1, float *pfc1bias,
                //  float *pfc2, float *pfc2bias,
                //  float *gfc1, float *gfc1bias,
                ...
                //         if(proc_id == i + NUM_WORKERS_DATA_PARALLELISM)
                //             // d_loss_all[i] = d_loss;
                //             copyData<<<1, 1>>>(d_loss, d_loss_all + i, 1);
                //         else
                //         {
                ...
                //     AccumulateLoss<<<1, 1>>>(d_loss_all, d_loss_accumulated, NUM_WORKERS_DATA_PARALLELISM);
                //     // printf("loss: %f\n", d_loss_sum);
                ...
                fpass_elap = e_fpass - s_fpass;
                fpass_time += fpass_elap.count();
                bp_elap = e_bp - s_bp;
                bp_time += bp_elap.count();
                fc1_elap = e_fc1 - s_fc1;
                fc1_time += fc1_elap.count();
                dfc1_elap = e_dfc1 - s_dfc1;
                dfc1_time += dfc1_elap.count();
                pool_elap = e_pool - s_pool;
                pool_time += pool_elap.count();

                ReduceLossToPlotVariable<<<1, 1>>>(d_loss_accumulated, d_loss_sum, (int)((iter + 1) / TEST_INTERVAL), 1);
            }
            len = fc1.pneurons.size();
            ReduceGradients<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_gfc1_all, d_gfc1, len, NUM_WORKERS_DATA_PARALLELISM);
            len = fc1.pbias.size();
            ReduceGradients<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_gfc1bias_all, d_gfc1bias, len, NUM_WORKERS_DATA_PARALLELISM);
            len = fc2.pneurons.size();
            ReduceGradients<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_gfc2_all, d_gfc2, len, NUM_WORKERS_DATA_PARALLELISM);
            len = fc2.pbias.size();
            ReduceGradients<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_gfc2bias_all, d_gfc2bias, len, NUM_WORKERS_DATA_PARALLELISM);
            fc_context.UpdateWeights(STEP_SIZE, NUM_WORKERS_DATA_PARALLELISM, d_pfc1, d_pfc1bias, d_pfc2, d_pfc2bias, d_gfc1, d_gfc1bias, d_gfc2, d_gfc2bias);
            
            e_fc = std::chrono::high_resolution_clock::now();
            fc_elap = e_fc - s_fc;
            fc_time = fc_elap.count();

            PutValToVector<<<1, 1>>>(fc_time, d_fc_time_vector, (int)(iter + 1) / TEST_INTERVAL);
            PutValToVector<<<1, 1>>>(fpass_time, d_fpass_time_vector, (int)(iter + 1) / TEST_INTERVAL);
            PutValToVector<<<1, 1>>>(bp_time, d_bp_time_vector, (int)(iter + 1) / TEST_INTERVAL);
            PutValToVector<<<1, 1>>>(fc1_time, d_fc1_time_vector, (int)(iter + 1) / TEST_INTERVAL);
            PutValToVector<<<1, 1>>>(dfc1_time, d_dfc1time_vector, (int)(iter + 1) / TEST_INTERVAL);
            PutValToVector<<<1, 1>>>(pool_time, d_pool_time, (int)(iter + 1) / TEST_INTERVAL);
        }

         checkCudaErrors(cudaMemcpy(h_fc_time_vector, &d_fc_time_vector[0], sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL), cudaMemcpyDeviceToHost));
        std::string ff = "fc_time_proc_id_";
        std::string fmt = ".txt";
        std::ofstream myfile ((ff + std::to_string(proc_id) + fmt).c_str());
        if (myfile.is_open())
        {
            for(int idx = 0; idx < (int)(ITERATIONS / TEST_INTERVAL); idx++)
            {
                myfile << h_fc_time_vector[idx] << "\n";
            }
            myfile.close();
        }
        checkCudaErrors(cudaMemcpy(h_fpass_time_vector, &d_fpass_time_vector[0], sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL), cudaMemcpyDeviceToHost));
        ff = "fpass_time_proc_id_";
        std::ofstream myfile1 ((ff + std::to_string(proc_id) + fmt).c_str());
        if (myfile1.is_open())
        {
            for(int idx = 0; idx < (int)(ITERATIONS / TEST_INTERVAL); idx++)
            {
                myfile1 << h_fpass_time_vector[idx] << "\n";
            }
            myfile1.close();
        }
        checkCudaErrors(cudaMemcpy(h_bp_time_vector, &d_bp_time_vector[0], sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL), cudaMemcpyDeviceToHost));
        ff = "bp_time_proc_id_";
        std::ofstream myfile2 ((ff + std::to_string(proc_id) + fmt).c_str());
        if (myfile2.is_open())
        {
            for(int i = 0; i < (int)(ITERATIONS / TEST_INTERVAL); i++)
            {
                myfile2 << h_bp_time_vector[i] << "\n";
            }
            myfile2.close();
        }

        checkCudaErrors(cudaMemcpy(h_fc1_time_vector, &d_fc1_time_vector[0], sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL), cudaMemcpyDeviceToHost));
        ff = "fc1_time_proc_id_";
        std::ofstream myfile3 ((ff + std::to_string(proc_id) + fmt).c_str());
        if (myfile3.is_open())
        {
            for(int idx = 0; idx < (int)(ITERATIONS / TEST_INTERVAL); idx++)
            {
                myfile3 << h_fc1_time_vector[idx] << "\n";
            }
            myfile3.close();
        }
        checkCudaErrors(cudaMemcpy(h_dfc1time_vector, &d_dfc1time_vector[0], sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL), cudaMemcpyDeviceToHost));
        ff = "dfc1_time_proc_id_";
        std::ofstream myfile4 ((ff + std::to_string(proc_id) + fmt).c_str());
        if (myfile4.is_open())
        {
            for(int idx = 0; idx < (int)(ITERATIONS / TEST_INTERVAL); idx++)
            {
                myfile4 << h_dfc1time_vector[idx] << "\n";
            }
            myfile4.close();
        }
        checkCudaErrors(cudaMemcpy(h_pool_time, &d_pool_time[0], sizeof(float) * (int)(ITERATIONS / TEST_INTERVAL), cudaMemcpyDeviceToHost));
        ff = "pool_time_proc_id_";
        std::ofstream myfile5 ((ff + std::to_string(proc_id) + fmt).c_str());
        if (myfile5.is_open())
        {
            for(int i = 0; i < (int)(ITERATIONS / TEST_INTERVAL); i++)
            {
                myfile5 << h_pool_time[i] << "\n";
            }
            myfile5.close();
        }
      ...
      if(proc_id == NUM_WORKERS_DATA_PARALLELISM)
      {
          ...
          std::ofstream myfile6 ("training_loss.txt");
          if (myfile6.is_open())
          {
              for(int i = 0; i < (int)(ITERATIONS / TEST_INTERVAL); i++)
              {
                  myfile6 << h_loss_sum[i] << "\n";
              }
              myfile6.close();
          }
      }
