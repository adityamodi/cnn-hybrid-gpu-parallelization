...
__global__ void ComputeLoss(const float *labels, const float *predictions, float *loss, int start_label, int my_class_size, int batch_size)
{
    ...
    if(labels[img_idx] == start_label + idx % my_class_size)
        loss[img_idx] = -log(predictions[img_idx + idx % my_class_size]);
}
...
__global__ void ArgMax(const float *predictions, float *returned_predictions, int my_class_size, int my_class)
{
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    int max_idx = -1;
    float max_value = -1;
...
int main(int argc, char** argv)
{
    ...
    {   
        ...
        float *d_test_data, *d_test_labels;
        ...
        // checkCudaErrors(cudaMemcpyAsync(d_test_data, &test_images_float, sizeof(float) * test_size * img_channels * img_height * img_width, cudaMemcpyHostToDevice));
        // checkCudaErrors(cudaMemcpyAsync(d_test_labels, &test_labels_float, sizeof(float) * test_size, cudaMemcpyHostToDevice));
        ...
        for(int iter = 0; iter < ITERATIONS; ++iter)
        {
            ...
            {
                conv_context.ForwardPropagation(d_test_data, d_conv1, d_pool1, d_conv2, d_pool2, d_pconv1, d_pconv1bias, d_pconv2, d_pconv2bias, d_cudnn_workspace);
                ...
                for(int i=0;i<NUM_WORKERS_DATA_PARALLELISM;i++){
                  ...
                  MPI_Isend(d_pool2, test_size * conv2.out_channels * (conv2.out_height / pool2.stride) * (conv2.out_width / pool2.stride), MPI_FLOAT, curr_FC_addr, FORPROP_TAG, MPI_COMM_WORLD, &req[i]);
    ...
    if(proc_id >= NUM_WORKERS_DATA_PARALLELISM && proc_id < num_processors)
    {
        ...
        float *d_pool2_fc, *d_labels_fc, *d_labels_all, , *d_pool_all;
        ...
        checkCudaErrors(cudaMalloc(&d_loss_all, sizeof(float) * NUM_WORKERS_DATA_PARALLELISM * fc_context.m_batchsize));
        checkCudaErrors(cudaMalloc(&d_loss_sum, sizeof(float)));
        ...
        checkCudaErrors(cudaMalloc(&accuracy, sizeof(float)));
        ...
        for (int iter = 0; iter < ITERATIONS; ++iter){
            ...
            if((iter + 1) % TEST_INTERVAL == 0)
            {
                ...
                for(int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++ ){
                    ...
                    MPI_Irecv(d_labels_fc + i * TEST_SET_SIZE, TEST_SET_SIZE, MPI_FLOAT, i, FORPROP_LABELS_TAG, MPI_COMM_WORLD, &req_labels[i]);
                }
                ...
                for(int i = 0; i < TEST_SET_SIZE; i++){
                    ArgMax<<<1, 1>>>(d_fc2smax + i * my_class_size, d_mypredictions, my_class_size, my_class);
                    if(proc_id != NUM_WORKERS_DATA_PARALLELISM)
                    {   
                    ...
                    if(proc_id == NUM_WORKERS_DATA_PARALLELISM)
                    {
                        ComputeZeroOneLoss<<<1, 1>>>(d_predictions_all, d_labels_fc + i, d_accuracy);
                    }
            ...
            for(int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++ ){
                ...
                // MPI_Recv(d_pool2_fc + i * len, len, MPI_FLOAT, i, FORPROP_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                // MPI_Recv(d_labels_fc + i * fc_context.m_batchsize, MPI_FLOAT, i, FORPROP_LABELS_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Irecv(d_pool_all + i * len, len, MPI_FLOAT, i, FORPROP_TAG, MPI_COMM_WORLD, &req_conv_activations[i]);  // Changed
            ...
            for(int j=0; j < NUM_WORKERS_DATA_PARALLELISM; j++){
                MPI_Wait(&req_conv_activation[j], MPI_STATUS_IGNORE);
                MPI_Wait(&req_labels[j], MPI_STATUS_IGNORE);
                len = fc_context.m_batchsize * CONV2_FILTERS * CONV2_OUTPUT_DIM * CONV2_OUTPUT_DIM;
                copyData<<<RoundUp(len, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_pool_all + j * len, d_pool2_fc, len);
                copyData<<<RoundUp(fc_context.m_batchsize, BLOCK_WIDTH), BLOCK_WIDTH>>>(d_labels_all + j * len, d_labels_fc, fc_context.m_batchsize);
                fc_context.ForwardPropagationFullyConnectedLayer(d_pool2_fc, d_fc1, d_fc1relu, d_pfc1, d_pfc1bias, d_onevec);
                ...
                if(proc_id == NUM_WORKERS_DATA_PARALLELISM){
                    ...
                    ReduceLoss<<<1, 1>>>(d_loss_all, d_loss_sum, NUM_WORKERS_DATA_PARALLELISM);
                    // printf("loss: %f\n", d_loss_sum);
