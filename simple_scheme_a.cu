...
__global__ void copyData(const float *data1, const float *data2, int len){
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
...
struct TrainingContextFullyConnectedLayer
{
    ...
    void ForwardPropagationPreSoftmaxLayer(float *fc1relu, float *fc2, float *result, float *pfc2, float *pfc2bias, float *onevec)
    {
        float alpha = 1.0f, beta = 0.0f;
        // FC2 layer
        ...
        checkCudaErrors(cublasSgemm(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N,
                                    ...
                                    &alpha,
                                    fc2, ref_fc2.outputs));
        ComputeLogisticActivations<<<10000, 512>>>(fc2, ref_fc2.outputs, m_batchsize, result);
...
int main(int argc, char** argv)
{
    ...
    if(proc_id < NUM_WORKERS_DATA_PARALLELISM)
    {
        ...
        for(int iter = 0; iter < ITERATIONS; ++iter)
        {
            ...
            // shouldnt this be MPI_STATUS_IGNORE?
            MPI_Waitall(NUM_WORKERS_DATA_PARALLELISM, req, stat);
            MPI_Waitall(NUM_WORKERS_DATA_PARALLELISM, req_labels, stat);
        }
    }
    if(proc_id >= NUM_WORKERS_DATA_PARALLELISM && proc_id < num_processors)
    {
        ...
        checkCudaErrors(cudaMalloc(&d_pool2_fc,   sizeof(float) * fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM * CONV2_FILTERS * CONV2_OUTPUT_DIM * CONV2_OUTPUT_DIM));
        ...
        checkCudaErrors(cudaMalloc(&d_labels_fc,   sizeof(float) * fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM));
        ...
        auto t1 = std::chrono::high_resolution_clock::now();
        for (int iter = 0; iter < ITERATIONS; ++iter){
            // Call GPU kernel for copying fc1 to fc1_all
            for(int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++ ){
            ...
            for( int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++){
                ...
                MPI_Isend(d_fc1relu, len, MPI_FLOAT, i + NUM_WORKERS_DATA_PARALLELISM, FORPROP_TAG, MPI_COMM_WORLD, &req[i]);
            }
            for( int i = 0; i < NUM_WORKERS_DATA_PARALLELISM; i++){
                len  = fc_context.m_batchsize * NUM_WORKERS_DATA_PARALLELISM * (FC1_OUTPUT_DIM / NUM_WORKERS_DATA_PARALLELISM);
                if( proc_id == i + NUM_WORKERS_DATA_PARALLELISM ) copyData<<<10, 512>>>(d_fc1relu, d_fc1_all + i * len, len); // call copy kernel
                MPI_Recv(d_fc1_all + i * len, len, MPI_FLOAT, i + NUM_WORKERS_DATA_PARALLELISM, FORPROP_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
            fc_context.ForwardPropagationPreSoftmaxLayer(d_fc1_all, d_fc2, d_fc2smax, d_pfc2, d_pfc2bias, d_onevec);
